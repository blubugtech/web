...
export default function TechnicalGuidesPage() {
  ...
  useEffect(() => {
    apiClient.get('/api/guides?size=100')
      .then(res => setAllGuides(res.data.content || []))
      .catch(console.error);
  }, []);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Practical tutorials and insights on software architecture, DevOps, and full-stack development.
          </p>
        </motion.div>
        ...
        >
          {allGuides.length > 0 ? (
            allGuides.map((guide, idx) => (
              ...
              >
                {guide.category && (
                  <div className="absolute top-0 right-0 bg-primary/20 text-primary border-b border-l border-primary/20 text-xs font-bold px-3 py-1 rounded-bl-lg">
                    {guide.category}
                  </div>
                )}
                <div className="flex flex-col h-full justify-between relative z-10">
                  <div>
                    ...
                    {guide.difficulty && guide.readingTimeMinutes && (
